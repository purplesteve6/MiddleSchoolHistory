/* ============================================================
   United States - Stacked Challenge 4
   Reuses the existing United States SVG, CSS, flags, and shared engine.
   Each included region has its own starting color; unused states are gray.
   ============================================================ */

window.MAP_CHALLENGE_CONFIG = {
  slug: "stack-challenge-4",

  svgPath: "./united-states-clickable.svg",

  showFlags: true,
  flagsBase: "/games/map-challenges/united-states/flags/",
  flagExt: ".webp",

  // Only states in the accumulated regions are playable.
  targets: [
    "alabama",
    "arkansas",
    "florida",
    "georgia",
    "kentucky",
    "louisiana",
    "mississippi",
    "north_carolina",
    "south_carolina",
    "tennessee",
    "virginia",
    "west_virginia",
    "delaware",
    "maryland",
    "new_jersey",
    "new_york",
    "pennsylvania",
    "connecticut",
    "maine",
    "massachusetts",
    "new_hampshire",
    "rhode_island",
    "vermont",
    "illinois",
    "indiana",
    "michigan",
    "minnesota",
    "ohio",
    "wisconsin",
    "iowa",
    "kansas",
    "missouri",
    "nebraska",
    "north_dakota",
    "south_dakota"
  ],

  // Each active region has its own starting color.
  // States not yet included in this level remain gray and non-playable.
  startColors: [
    {
      color: "#C97969",
      ids: [
        "alabama",
        "arkansas",
        "florida",
        "georgia",
        "kentucky",
        "louisiana",
        "mississippi",
        "north_carolina",
        "south_carolina",
        "tennessee",
        "virginia",
        "west_virginia"
      ]
    },
    {
      color: "#D2A348",
      ids: [
        "delaware",
        "maryland",
        "new_jersey",
        "new_york",
        "pennsylvania"
      ]
    },
    {
      color: "#58A39B",
      ids: [
        "connecticut",
        "maine",
        "massachusetts",
        "new_hampshire",
        "rhode_island",
        "vermont"
      ]
    },
    {
      color: "#6F9FC8",
      ids: [
        "illinois",
        "indiana",
        "michigan",
        "minnesota",
        "ohio",
        "wisconsin"
      ]
    },
    {
      color: "#A7A95F",
      ids: [
        "iowa",
        "kansas",
        "missouri",
        "nebraska",
        "north_dakota",
        "south_dakota"
      ]
    },
    {
      color: "#777777",
      ids: [
        "alaska",
        "arizona",
        "california",
        "colorado",
        "hawaii",
        "idaho",
        "montana",
        "nevada",
        "new_mexico",
        "oklahoma",
        "oregon",
        "texas",
        "utah",
        "washington",
        "wyoming"
      ]
    }
  ],

  ignoreIds: [
    "water",
    "borders",
    "context_land"
  ],

  displayNames: {
    new_york: "New York"
  },

  alias: {},
  groups: {},
  extraIds: [],

  ui: {
    bannerTitle: "UNITED STATES STACKED CHALLENGE 4",
    bannerAria: "United States stacked map challenge 4 banner",

    mainAria: "United States stacked map challenge 4",
    mapAria: "United States map",

    overlayKicker: "STACKED MAP CHALLENGE",
    overlayTitle: "UNITED STATES | LEVEL 4",
    beginMessage: "Southeast + Mid-Atlantic + New England + Midwest + Great Plains",

    logoSrc: "/assets/images/logo/MSHistory_Logo_Small.png"
  }
};
